...
* **Generate 'cty' claim (forced for nested JWT/JWS)**: If selected then the cty header parameter is generated by examining the Content-Type of the attribute referenced by the selector. See [JWS RFC 7515](https://tools.ietf.org/html/rfc7515#section-4.1.10) for an explanation of the cty header.

#### Generate JWT

Select this option to create a JWT from a JSON template. A standard set of JWT Claims can be added to the template to produce the JWT. You can find a detailed explanation for each claim at [JWT RFC 7519](https://tools.ietf.org/html/rfc7519#section-4.1). This option also supports OpenID specific claims claims 'at_hash' and 'c_hash', see the [Open ID Connect Core Specification](https://openid.net/specs/openid-connect-core-1_0.html) for more information on these claims.

* **JSON Template**: A JSON object containing a JWT claims set. This could be used to specify Public Claim Names and/or Private Claim Names.

The following allows to specify the contents of the Registered Claim Names:

* **Issuer 'iss'**: A selector can be used to specify the iss claim. If the selector evaluates as empty or null, the claim will not be added to the JWT.
* **Subject 'sub'**: A selector can be used to specify the sub claim. If the selector evaluates as empty or null, the claim will not be added to the JWT.
* **ID 'jti**': A selector can be used to specify the jti claim. If the selector evaluates as empty or null, the claim will not be added to the JWT.
...
* **Include system time 'iat'**: If selected, then the issued at time (iat) is included in the generated payload, this will be the systems Unix epoch time in seconds.

The following allow to add claims to support OpenID Connect.

* **Access Token (generate 'at_hash' half hash)**: A selector can be used to specify the Access Token used to generate the at_hash claim. If the selector evaluates as empty or null, the claim will not be added to the JWT.
* **Authorization Code (generate 'c_hash' half hash)**: A selector can be used to specify the Authorization Code used to generate the c_hash claim. If the selector evaluates as empty or null, the claim will not be added to the JWT.
